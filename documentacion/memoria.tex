...
\end{figure}
	...
	En esta sección se comentan dos técnicas que serán interesantes y útiles para el proyecto. El código Zl va a transformarse a código en JavaScript, que es un lenguaje cuyo entorno de ejecución está fuertemente basado en un sistema por eventos. Sin embargo, Zl no lo es. Para trasformar el código de Zl con un único hilo de ejecución a JavaScript con un sistema basado en eventos se va a utilizar una utilidad que se introdujo en 2017 en JavaScript, comúnmente llamada `async/await'.
	
	Por otro lado, en JavaScript se controlan los errores y las excepciones usando la sintaxis `try-catch', sin embargo, en Zl no existe tal sintaxis. Por lo tanto, la forma en la que se le comunica al programador de que ha sucedido un error va a ser distinta, y se se inspira en la forma que tiene GoLang (un lenguaje de programación donde las funciones pueden devolver más de un valor) para controlar los errores.

		\vspace{10px}
	
	Un ejemplo de subrutina en Zl que, al compilarse a JavaScript, se controla por eventos y además puede fallar (y habría que controlar el error en caso de fallo) es la siguiente:
...
\end{BVerbatim}
	...
	Por ejemplo, si un usuario quiere cargar una imagen en JavaScript, lo correcto sería hacer la petición para obtener la imagen, y controlar los eventos `onload' y `onerror'. En Zl la llamada se realiza, \textbf{bloqueando} la ejecución del código en este punto hasta que, internamente, uno de los dos eventos acaba (onload u onerror). 
	...
	Por un lado, se define el objeto `Promise' \cite{javascript-promise}. Este objeto ofrece al programador que lo crea dos funciones: \textbf{resolve} y \textbf{reject}. 
	
	\vspace{10px}
	
	Cuando un evento asíncrono ha salido bien, el programador podrá llamar a `resolve' indicando que la promesa ha sido \textbf{resuelta} correctamente. Si por el contrario, ha habido un error durante el evento, o el evento ha fallado, el programador debe llamar a `reject' para indicar que la promesa ha sido \textbf{rechazada}. 
	
	\vspace{10px}
	
	Por otro lado, se definen las funciones asíncronas. Éstas son aquellas que, o son declaradas con la palabra reservada `async', y devuelven un objeto `Promise'. `async' es una palabra reservada que se coloca delante de la palabra `function' en JavaScript, indicando que la función devolverá una promesa.
	
	\vspace{10px}
	
	Por otro lado, las funciones que son asíncronas se pueden llamar usando la palabra reservada `await' siempre que se esté dentro de una función asíncrona también. Cuando son llamadas de esta forma, el código se bloquea hasta que la promesa se \textbf{resuelve} o se \textbf{rechaza}.
...
\end{BVerbatim}
	...
	\subsection{Tratamiento de la no-factorización}
	
	 Para el tratamiento de la no-factorización el método sugerido es avanzar paso a paso los primeros elementos que hacen ambigua las distintas opciones. Si tomamos un ejemplo de no-factorización donde solo se puede saber si debemos avanzar o no después de leer el segundo token, el algoritmo a seguir con las funcionalidades del analizador es el siguiente:
	
\begin{BVerbatim}
; Esta regla no está factorizada porque aunque se lea 
